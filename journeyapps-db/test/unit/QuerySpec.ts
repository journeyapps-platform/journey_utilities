...
import { DBType } from '../../dist/types/Type';
import { DBObjectType } from '../../dist/types/ObjectType';
import {
  DBDatetimeType,
  DBDateType,
  DBIntegerType,
...
describe('Queries', function () {
  ...
  const colour = new Variable('colour', new DBTextType());
  const colours = new Variable('colours', new DBMultipleChoiceType());
  colours.type.addOption('red', 'Red', 0);
  colours.type.addOption('green', 'Green', 1);
  colours.type.addOption('blue', 'Blue', 2);
  const number = new Variable('number', new DBIntegerType());
  const date = new Variable('date', new DBDateType());
  const day = new Variable('day', new DBDateType());
  day.type.isDay = true;
  const datetime = new Variable('datetime', new DBDatetimeType());
  const ownerId = new Variable('owner_id', new DBTextType());
  const owner = new Variable('owner', new DBObjectType());
  ownerId.relationship = 'owner';
  owner.relationship = 'owner';

  beforeEach(function () {
    scope = new DBType(null);
    scope.name = 'TestScope';
    scope.addAttribute(new Variable('id', new DBTextType()));
    scope.addAttribute(new Variable('make', new DBTextType()));
    scope.addAttribute(new Variable('model', new DBTextType()));
    scope.addAttribute(new Variable('score', new DBIntegerType()));
    scope.addAttribute(new Variable('date', new DBDateType()));
    scope.addAttribute(new Variable('datetime', new DBDatetimeType()));
  ...
  describe('Expression parsing', function () {
    ...
    it('Should parse basic expressions', function () {
      ...
      expect(expression.attributes()).toEqual([scope.getAttribute<DBType>('id')]);
      ...
      const id = scope.getAttribute<DBType>('id');
      const score = scope.getAttribute<DBType>('score');
