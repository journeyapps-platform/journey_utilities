...
import { Variable, Type, ObjectType } from '@journeyapps/parser-schema';
...
export function expressionFromHash(scopeType: Type, hash: any): Expression {
  ...
  for (let key in hash) {
    if (hash.hasOwnProperty(key)) {
      let value = hash[key];
      const attribute = scopeType.getAttribute(key) as Variable<DBType>;
      if (attribute == null) {
  ...
  */
  ...
  private parseTerm(): Expression {
    if (this.tokenizer.isWord()) {
      ...
      const attribute = this.scopeType.getAttribute(attributeName) as Variable<DBType>;
      if (attribute == null) {
