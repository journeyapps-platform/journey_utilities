...
import { AttachmentType, ChoiceType, ObjectType, Relationship, Schema, Variable } from '@journeyapps/parser-schema';
...
export class JourneyAPIAdapter extends BaseAdapter {
  static apiToInternalFormat(type: ObjectType, apiObj: any) {
    ...
    forEach(type.attributes, (attr: Variable) => {
      const name = attr.name;
      if (AttachmentType.isInstanceOf(attr.type)) {
        if (apiObj[name + '_id']) {
          const attachment = {
            id: apiObj[name + '_id'],
            state: 'pending',
      ...
      } else if (ChoiceType.isInstanceOf(attr.type) && attr.type.hasOptions) {
  ...
  async executeQuery(query: Query): Promise<ObjectData[]> {
    ...
    return await this.executeQueryGetMore(query, results, userSpecifiedLimit);
  }
  ...
  private async getIdBatch(model: ObjectType, ids: string[]) {
    const idAttr = model.getAttribute('id') as Variable<DBType>;
  ...
  async applyBatch(crud: ExecuteBatchOperation[]) {
    ...
    for (let i = 0; i < crud.length; i++) {
      ...
      if (action == 'put') {
        actions.push({
          method: 'put',
          object: dbToApiObject(this.schema.getType(operation.type) as ObjectType, operation.id, operation.data)
        });
      } else if (action == 'patch') {
        actions.push({
          method: 'patch',
          object: dbToApiObject(this.schema.getType(operation.type) as ObjectType, operation.id, operation.patch)
        });
...
function dbToApiObject(type: ObjectType, id: string, dbObj: PersistedObjectData) {
  ...
  for (let key of Object.keys(dbObj.attributes)) {
    ...
    if (attrType != null && AttachmentType.isInstanceOf(attrType)) {
      if (value?.id) {
