...
export class DatabaseObject {
  ...
  static build<T = {}>(adapter: DatabaseAdapter, type: ObjectType, id?: string, data?: any): DatabaseObject & T {
    const obj = new DatabaseObject(adapter, type, id);
    if (data != null) {
      obj.resolve(data);
    }
  ...
  constructor(adapter: DatabaseAdapter, type: ObjectType, id: string) {
        ...
        Object.keys(type.attributes).forEach((key) => {
          ...
          if (!attributesDirty[key]) {
            ...
            if (value == null) {
              ...
              realValue = attribute.type.valueFromJSON(value);
            }
    ...
    Object.keys(type.belongsTo).forEach(function (name) {
      ...
      const accessor = createAccessor(name, setBelongsTo, getBelongsTo);
...
export let createAccessor: CreateAccessor = (name, set, get) => {
  return (value?: any) => {
